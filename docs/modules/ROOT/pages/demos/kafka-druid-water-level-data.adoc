...
The records get produced (put in) by the data ingestion job and are consumed (read) by Druid afterwards in the same order they where produced.
...
The `Avg. row size (bytes)` shows, that a typical `measurement` record, while a `station` record has 213 bytes, which is 50 times the size.
So with choosing two dedicated topics over a single topic the demo was able to save 50x of storage and compute costs.

By clicking on the `measurements` datasource you can see the segments the datasource consists of.
In this case the `measurements` datasource is partitioned by the day of the measurement, resulting in 33 segments.
...
You can also find out the number of measurements for every water:
...
Open the `minio` endpoint `console-http` in your browser (http://172.18.0.5:32595 in this case).
...
You can see the druid has created a folder for both of the two datasources.
...
One prefix corresponds to on segment which in turn contains all the measurements of a day.
...
If you open up a prefix for a specific day you can see that Druid has placed a file containing the data of this day there.
