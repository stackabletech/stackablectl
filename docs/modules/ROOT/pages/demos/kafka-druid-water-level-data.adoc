...
The `Avg. row size (bytes)` shows, that a typical `measurement` record has is `4` bytes large, while a `station` record has `213` bytes, which is more than 50 times the size.
...
They are colored by the water they measure, so all stations of a water have the same color.
