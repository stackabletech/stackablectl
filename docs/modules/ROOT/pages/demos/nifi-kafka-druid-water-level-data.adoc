...
It is split in two main components:
...
You can zoom in by using you mouse and mouse wheel.

image::demo-nifi-kafka-druid-water-level-data/nifi_3.png[]
image::demo-nifi-kafka-druid-water-level-data/nifi_4.png[]

Detailed view of the top and bottom section.

The `Get station list` processors fet the current list of stations as JSON via HTTP from the PEGELONLINE web service.
Two downstream processors consume this list:
...
7. `PublishKafkaRecord_2_6` finally emits every measurement as a Kafka records to the topic `measurements`.

The right side works nearly the same way, but is executed in an endless loop to stream the data in near-realtime.
...
Click on the `Remote URL` setting.
...
Afterwards you can see that ~10 million records got produced in ~5 mins, which corresponds to ~30k measurements/s.
Keep in mind that the demos uses a single-node NiFi setup, the performance could have been increased.
