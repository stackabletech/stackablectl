...
** *Superset*: A modern data exploration and visualization platform. This demo uses it to execute SQL queries on Trino and build dashboards
...
You can have a look at the testdata within the MinIO Webinterface by opening the given `minio` endpoint `console-http` from your `stackablectl services list` command output.
