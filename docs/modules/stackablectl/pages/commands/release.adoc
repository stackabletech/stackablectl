...
The operators maintain their own CRDs and update them automatically on startup, so no manual CRD update is required (see xref:home:concepts:maintenance/crds.adoc[]).
